...
- **`contact/index.html`** (serves `/contact`) — nav → hero ("ready to build?" + one-line intro) → 3-pillar trust cluster → native enquiry form ("Tell me what you need") → footer. Loads `contact/contact-form.js` (`defer`). Unlike the other main pages its markup is minified onto a few long lines and styled by class-based CSS in `<head>` rather than inline `style=""`. See Key Patterns → Contact.
- Two supporting pages in the same `<x-dc>` format, not in the nav: **`thank-you/index.html`** (where the form redirects after a successful submit; `noindex,nofollow`, not in the sitemap) and **`privacy/index.html`** (Privacy Notice, linked from the form; in the sitemap).
...
**Contact — native enquiry form (replaced the two Airtable iframe embeds in `bcbebb7`/`f17d683`):** one `<form id="contact-form">` that submits JSON to `/api/contact`.
- **The endpoint is not in this repo.** `/api/contact` is served by Cloudflare in front of GitHub Pages (responses carry no `x-github-request-id`); it accepts `POST` only (`GET` → `405 {"error":"method_not_allowed"}`). GitHub Pages alone can't run it, so the form doesn't work under a local `python -m http.server` — expect a failed POST locally. Don't submit test enquiries against the live endpoint; they're real messages.
- **Fields:** required — `name` (2–100 chars), `email` (≤254, basic `x@y.z` check), `enquiry_type` (`project_discussion` / `pricing_process` / `general_enquiry`), `message` (20–5000). Optional — `company` (≤150), `current_tools` (≤2000), `team_size`, `budget_range`, `start_timeframe` (fixed option values; the JS whitelists them and the endpoint presumably does too, so keep `<option value>`s, the `choices` sets in `contact-form.js`, and the server in sync). Hidden: `privacy_notice_version` (`2026-09-13` — also hard-coded in `contact-form.js`; bump both if the Privacy Notice changes materially) and `source_path`. Honeypot: `website_trap` inside `.honeypot` (`aria-hidden`, `tabindex=-1`) — leave it empty and hidden.
- **`contact/contact-form.js` owns submission:** client-side validation with per-field messages in `#error-<name>` (`aria-live`) and `aria-invalid`; adds UTM params from the URL, `source_path` and an `idempotency_key` (UUID kept in `sessionStorage` until success, so retries don't duplicate); handles `429` (rate-limited), server `fieldErrors`, generic and network failures in `#form-status`; on success fires Umami event `contact-form-submitted` and redirects to `/thank-you/`. It also tracks clicks as Umami events: `direct-email-click`, `direct-phone-click`, `professional-profile-upwork`, `professional-profile-contra`. It binds on `document` (survives the runtime re-render) and re-applies `required`/`aria-required` after load for the same reason.
- **Two submit handlers exist:** an older inline `<script>` at the end of the page's `<x-dc>` has its own validate-and-POST logic. `contact-form.js` registers its `submit` listener in the **capture** phase and calls `stopImmediatePropagation()`, so it always wins and the inline one only runs if `contact-form.js` fails to load. Change behaviour in `contact-form.js`; if you change field rules, update the inline fallback too or remove it deliberately.
- **Stale anchors:** there are no `#build` / `#pricing` anchors any more, but `/contact/#build` is still linked from `index.html` ("Let's Build"), `about/index.html` and `portfolio/index.html`, and `/contact/#pricing` from `services/index.html`. They land at the top of the page (harmless); point them at `/contact/` or add a matching `id` when next touching those pages.

**Contact — Trust Cluster (3-pillar):** Single glass panel, `flex: 1` columns separated by 1px dividers. Sits directly under the page hero, above the enquiry form.
